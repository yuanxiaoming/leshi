...
/**
 * Created by shalafi on 6/6/13. &&listView转变为gridview
 */
public abstract class ListAsGridBaseAdapter extends BaseAdapter {
    ...
    private LinearLayout createItemRow(int position, ViewGroup viewGroup, int columnWidth) {
        ...
        int adjustedColumnWidth = columnWidth
                - (layout.getPaddingLeft() + layout.getPaddingRight()) / mNumColumns;
    ...
    private void updateItemRow(int position, ViewGroup viewGroup, LinearLayout layout,
            int columnWidth) {
        int realColumnWidth = columnWidth - (layout.getPaddingLeft() + layout.getPaddingRight())
                / mNumColumns;
        for (int i = 0; i < mNumColumns; i++) {
            int currentPos = position * mNumColumns + i;
